...
trecords.prototype.searchstring=function() { return this.link+' '+this.keywords }
...
function searchAll(keyword){
	...
	if (finds==0) {
		document.write("<h3><font color=green>No se encontró <b>''"+keyword+"''</b></font></h3>"+
						...
						"<LI>Intente nuevamente utilizando otra palabra.</li>"+
						
						"<LI>Verifique que el contenido que este buscando este disponible.</li></span>"+
						"</p>")
		DisplayXSearch()
		document.close()
		return
	...
	for (q=display_start; q<q3; q+=1) {
		if (results[q].val>0) {
			...
			if (x1>0 && x2>0) {
				...
				document.write("<table border=0 width=500><tr><td><span class='xresult'>"+records[rc].description+"</span></td></tr></table><br>")
			}
			q2++
   		}
	}
